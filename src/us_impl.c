...
static void
put_prefix(_In_ UselessShell* us)
{
    if (us->current_user)
    {
        if (!NO_COLOR) put(ANSI_COLOR_CYAN);
...
static int
us_open_log(_In_ USLogFile* log)
{
...
static void
us_close_log(_Inout_ USLogFile* log)
{
    if (log->read)
    {
        fclose(log->read);
...
_Ret_maybenull_ static const ICommandProvider*
_find_cmd(_In_ const ICommandProvider* provider, _Inout_ const char** arg_start)
{
...
_Ret_maybenull_ static const USCommand*
find_cmd(_In_ const ICommandProvider* provider, _In_ const char* parts, _Out_ const char** arg_start)
{
    *arg_start = parts;
...
_Ret_maybenull_ static inline const USCommand*
us_find_cmd(_In_ UselessShell* us, _In_ const char* parts, _Out_ const char** arg_start)
{
    return find_cmd((const ICommandProvider*)us, parts, arg_start);
}

static void
_tab_complete(_Inout_ USTabCompleteQuery* tc, _In_ const ICommandProvider* provider)
{
    for (short i = 0; i < provider->cmds_len; i++)
    {
        const USCommand* cmd = &provider->cmds[i];
...
static void
tab_complete(_Inout_ USTabCompleteQuery* tc, _In_ const ICommandProvider* provider, _In_ char* search, int len)
{
...
static inline void
us_tab_complete(_Inout_ USTabCompleteQuery* tc, _In_ char* search, int len)
{
...
static void
_get_details(_Inout_ DetailsQuery* dq, _In_ const ICommandProvider* provider)
{
    for (short i = 0; i < provider->cmds_len; i++)
    {
        const USCommand* cmd = &provider->cmds[i];
...
static bool
get_details(_Inout_ DetailsQuery* dq, _In_ const ICommandProvider* provider, _In_z_ char* search, int len)
{
    char parts[32 * 16];
    dq->result[0] = 0;
...
static inline bool
us_get_details(UselessShell* us, DetailsQuery* dq, char* search, int len)
{
...
us_get_secret(_Out_writes_(max) char* buffer, byte min, byte max)
{
    ...
    while (1)
    {
        ...
        switch (c)
        {
        case USCharExit: put("\r" ANSI_CLEAR_PROCEEDING_LINE); return USReturnExit;
        ...
        case USCharEscape: put("\r" ANSI_CLEAR_PROCEEDING_LINE); return USReturnCancel;
        ...
        case '\t':       NOT_ALLOWED(); break;

        case USCharUndo: break;
        case USCharRedo: break;
...
static int
us_save_user(_In_ UselessShell* us, _In_ User* user)
{
...
_Ret_maybenull_ User*
us_create_user_h(
    _Inout_ UselessShell* us, _In_z_ const char* username, _In_ const char* password_hash, bool is_super, char icon
)
{
...
us_delete_user(_Inout_ UselessShell* us, _In_ User* user)
{
    ...
    create_directory(path);
    strcat(path, user->username);
    if (delete_directory(path) == 0) return true;
 ...
 */
_Success_(return) static int us_log_read_last(
    _Inout_ USLogFile* log, _Out_writes_(USInputBufferSize) char* buffer, _Out_ char** line_start, _Inout_ long* from_pos
)
{
 ...
 * @returns lenth of line
 */
_Success_(return) static int us_log_read_next(
    _Inout_ USLogFile* log, _Out_writes_(USInputBufferSize) char* buffer, _Out_ char** line_start, _Inout_ long* from_pos
)
{
...
static int
us_open_history(UselessShell* us)
{
...
{
    if (us->current_user && us_save_user(us, us->current_user)) return false;
    us_close_log(&us->history);
    us->current_user = NULL;
    strcpy(us->history.path, us->cfg_path);
...
us_process_cmd(_Inout_ UselessShell* us, int len)
{
    ...
    if (split_cmd(us->ibuff, len, parts))
    {
        ...
        if (cmd)
        {
            ...
            if (cmd->args_len)
            {
                f_args = malloc(cmd->args_len * sizeof(USCommandArgValue));
                assert(f_args);
                    ...
                    if (r_arg[0])
                    {
                        ...
                        else
                        {
                            ...
                            case USCommandArgTypeSecret:
                            {
                                ...
                                MD5Final(&md5);
                                char* str = malloc(sizeof(md5.digest));
                                assert(str);
                            ...
                            case USCommandArgTypeFlag: break;
                            }
                        }
            ...
            if (cmd->impl)
            {
                ...
                {
                case USCommandReturnExit:   ret = USProcessCmdExit; break;
                case USCommandReturnError:  ret = USProcessCmdExecError; break;
                case USCommandReturnNormal: break;
                }
            }
            ...
            if (cmd->args_len)
            {
                while (a--)
                {
                    ...
                    if (val->exists)
                    {
                        switch (arg->type)
                        {
                        case USCommandArgTypeNumber:  break;

                        case USCommandArgTypeString:
                        case USCommandArgTypeSecret:  free(val->string); break;
